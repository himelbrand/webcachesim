...
AdaptSizePlusCache::AdaptSizePlusCache()
    : LRUCache()
    , _cParam(1 << 15)
    , statSize(0)
    , _maxIterations(15)
...
void AdaptSizePlusCache::setPar(std::string parName, std::string parValue) {
    if(parName.compare("t") == 0) {
        const uint64_t t = stoull(parValue);
        assert(t>1);
        _reconfiguration_interval = t;
...
bool AdaptSizePlusCache::lookup(SimpleRequest* req)
{
    reconfigure(); 
...
void AdaptSizePlusCache::admit(SimpleRequest* req)
{
    double roll = _uniform_real_distribution(globalGenerator);
    double free_space = double(_cacheSize - _currentSize);
    double candidate_size = double(req->getSize());
    double relevant_size = std::abs(std::min(free_space - candidate_size, 0.0));
    double admitProb = std::exp(-1.0 * relevant_size/_cParam);
    
    if(roll < admitProb) 
        LRUCache::admit(req); 
}

void AdaptSizePlusCache::reconfigure() {
    --_nextReconfiguration;
    if (_nextReconfiguration > 0) {
        return;
    } else if(statSize <= getSize()*3) {
...
double AdaptSizePlusCache::modelHitRate(double log2c) {
    // this code is adapted from the AdaptSize git repo
    // github.com/dasebe/AdaptSize
    double old_T, the_T, the_C;
    double sum_val = 0.;
